...
public class MyGraph {
	...
	public static void main(String[] args) throws IOException {
		...
		// System.out.println(g.getNewString("WordsLink public s next
		// WordsLink"));
		// g.randomMove("path");
		// g.SaveAsPiture();
		...
		// 输出程序运行时间
		// g.calcShortestPath("a","z");

		// g.PrintBridgeWords("c", "a");
		// g.printPicture();
		System.out.println(g.queryBridgeWords("happy", "yes"));
	}
	...
	{
		String bridgewords = queryBridgeWords(fore, behi);
	 ...
	 * String[] arr = str.split(" "); String outcome = "" + arr[0]; List list;
	 * for (int i = 0; i < arr.length - 1; i++) { list =
	 * queryBridgeWords(arr[i], arr[i + 1]); if (list.size() == 0) { outcome =
	 * outcome + " " + arr[i + 1]; } else if (list.size() == 1) { outcome =
	 * outcome + " " + list.get(0) + " " + arr[i + 1]; } else { int j = new
	 * Random().nextInt(list.size() - 1); outcome = outcome + " " + list.get(j)
	 * + " " + arr[i + 1]; } } return outcome; }
	 */
}
