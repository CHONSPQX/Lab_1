...
public class MyGraph {
  ...
  public static void main(String[] args) throws IOException {
    ...
    MyGraph g = new MyGraph(
        FileTrans.trans("E:\\java\\JavaFXApplication1\\test.txt"));
  ...
  private String BuildBridgeDot(String head, String tail,
      ArrayList<String> bridgeword) throws IOException// 根据桥接词查询结果生成dot文件，返回dot文件名
  {
    FileOutputStream out = new FileOutputStream(
        head + "Bridge" + tail + ".dot");
  ...
  private void getPaths(HashMap<String, ArrayList<String>> pathinfo,
      String begin, String end, ArrayList<String> PathOutcome, String temppath)// 根据单源最短路径算法的计算结果，整理出所有最短路径
  {
  ...
  public String FindPictureOfPath(String path, int i)// 根据路径和数字，解析出这条路径所对应的GIF文件名
  {
    if (i >= 0) {
      ...
      return pathnodes.get(0) + "to" + pathnodes.get(pathnodes.size() - 1) + i
          + "Pic." + type;
    } else {
      String type = "gif";
      List<String> pathnodes = Arrays.asList(path.split(" "));
      return pathnodes.get(0) + "to" + pathnodes.get(pathnodes.size() - 1)
          + "Pic." + type;
    }
  }
  ...
  public String BuildPathDot(String path, int i) throws IOException// 根据path和数字，首先产生一个dot文件，然后将路径信息写入到dot文件中，返回dot文件名
  {
    ...
    if (i >= 0)
      out = new FileOutputStream(pathnodes.get(0) + "to"
          + pathnodes.get(pathnodes.size() - 1) + i + ".dot");
    else
      out = new FileOutputStream(pathnodes.get(0) + "to"
          + pathnodes.get(pathnodes.size() - 1) + ".dot");
    try (OutputStreamWriter writer = new OutputStreamWriter(out)) {
      ArrayList<WordNode> pos = null;
      writer.write("digraph gra{" + '\n');
      // writer.write("rankdir=LR\n");
      ...
      for (String label : words) {
        ...
        if (!pos.isEmpty()) {
          ...
          for (WordNode w : pos) {
            ...
            // writer.write("\""+label+"\""+" -> "+"\""+w.label+"\""+" [ label =
